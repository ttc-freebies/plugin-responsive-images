...
<?php else : ?>
	...
	<div class="com-newsfeeds-newsfeed newsfeed<?php echo $direction; ?>">
		...
		<?php if (isset($images->image_first) && !empty($images->image_first)) : ?>
			...
			<figure class="com-newsfeeds-newsfeed__first-image img-intro-<?php echo htmlspecialchars($imgfloat, ENT_COMPAT, 'UTF-8'); ?>">
				<?php
				echo LayoutHelper::render(
					'ttc.image',
					[
						'img' => '<img src="' . htmlspecialchars((HTMLHelper::cleanImageURL($images->image_first))->url,  ENT_QUOTES, 'UTF-8') . '" ' . $alt . '/>',
						'breakpoints' => [200, 320, 480, 768, 992, 1200, 1600, 1920]
					]
				);
				?>
				<?php if ($images->image_first_caption) : ?>
					<figcaption <?php echo 'class="caption"'; ?>><?php echo htmlspecialchars($images->image_first_caption); ?></figcaption>
				<?php endif; ?>
			</figure>
		<?php endif; ?>
		...
		<?php if (isset($images->image_second) and !empty($images->image_second)) : ?>
			...
			<?php $alt = empty($images->image_second_alt) && empty($images->image_second_alt_empty)
				...
				<figure class="com-newsfeeds-newsfeed__second-image img-intro-<?php echo htmlspecialchars($imgfloat, ENT_COMPAT, 'UTF-8'); ?> item-image">
					<?php
					echo LayoutHelper::render(
						'ttc.image',
						[
							'img' => '<img src="' . HTMLHelper::cleanImageURL(htmlspecialchars($images->image_second, ENT_COMPAT, 'UTF-8')) . '" ' . $alt . '/>',
							'breakpoints' => [200, 320, 480, 768, 992, 1200, 1600, 1920]
						]
					);
					?>
					<?php if ($images->image_second_caption) : ?>
						<figcaption <?php echo 'class="caption"'; ?>><?php echo htmlspecialchars($images->image_second_caption); ?></figcaption>
					<?php endif; ?>
				</figure>
			</div>
		<?php endif; ?>
		...
		<?php if ($this->rssDoc->image && $this->params->get('show_feed_image')) : ?>
			<div class="com-newsfeeds-newsfeed__feed-image">
				...
				echo LayoutHelper::render(
					'ttc.image',
					[
						'img' => '<img src="' . HTMLHelper::cleanImageURL(htmlspecialchars($this->rssDoc->image->uri, ENT_COMPAT, 'UTF-8')) . '" alt="' . htmlspecialchars($this->rssDoc->image->title, ENT_COMPAT, 'UTF-8') . '"/>',
						'breakpoints' => [200, 320, 480, 768, 992, 1200, 1600, 1920]
					]
				);
				?>
